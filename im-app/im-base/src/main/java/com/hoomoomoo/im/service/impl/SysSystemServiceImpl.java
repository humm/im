...
public class SysSystemServiceImpl implements SysSystemService {
    ...
    public void loadBusinessId() {
        ...
        RunDataConfig.BUSINESS_SERIAL_NO.clear();
        ...
        if (CollectionUtils.isNotEmpty(businessIdList)) {
            for (String businessId : businessIdList) {
                ...
                RunDataConfig.BUSINESS_SERIAL_NO.put(businessKey, businessValue);
            }
        }
    ...
    public String getBusinessSerialNo(String businessType) {
        ...
        lock.lock();
        try {
            businessId = RunDataConfig.BUSINESS_SERIAL_NO.get(businessType);
            // 业务ID不存在 设置默认值
            ...
            // 更新内存数据序列号值
            RunDataConfig.BUSINESS_SERIAL_NO.put(businessType, businessId);
    ...
    public void loadSysDictionaryCondition() {
        ...
        RunDataConfig.DICTIONARY_CONDITION.clear();
        // 查询用户信息
        ...
        for (SysUserModel sysUserModel : sysUserList) {
            for (SysDictionaryModel sysDictionaryModel : sysDictionaryList) {
                ...
                // 用户不存在
                if (RunDataConfig.DICTIONARY_CONDITION.get(sysUserModel.getUserId()) == null) {
                    setDictionaryItem(sysUserModel, sysDictionaryModel, userDataAuthority, new ConcurrentHashMap(1), new ConcurrentHashMap(1));
                } else {
                    // 用户存在 字典不存在
                    if (RunDataConfig.DICTIONARY_CONDITION.get(sysUserModel.getUserId()).get(dictionaryCode) == null) {
                        setDictionaryItem(sysUserModel, sysDictionaryModel, userDataAuthority,
                                RunDataConfig.DICTIONARY_CONDITION.get(sysUserModel.getUserId()),
                                RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(sysUserModel.getUserId()).append(BLANK).toString()));
                    } else {
                        // 用户存在 字典存在
                        if (isUserDictionary(sysUserModel, sysDictionaryModel, userDataAuthority)) {
                            RunDataConfig.DICTIONARY_CONDITION.get(sysUserModel.getUserId()).get(dictionaryCode).add(sysDictionaryModel);
                            RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(sysUserModel.getUserId()).append(BLANK).toString()).get(dictionaryCode).add(sysDictionaryModel);
                        }
                    }
                }
            }
        ...
        switch (viewData.getViewType()) {
            case BUSINESS_TYPE_INCOME:
                viewData.getCondition().put(SELECT_USER_ID,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D000));
                viewData.getCondition().put(SELECT_INCOME_COMPANY,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D005));
                viewData.getCondition().put(SELECT_INCOME_TYPE,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D003));
                break;
            case BUSINESS_TYPE_GIFT:
                viewData.getCondition().put(SELECT_GIFT_TYPE,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D004));
                viewData.getCondition().put(SELECT_GIFT_SENDER,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D009));
                viewData.getCondition().put(SELECT_GIFT_RECEIVER,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D009));
                break;
            case BUSINESS_TYPE_USER:
                viewData.getCondition().put(SELECT_USER_STATUS,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D001));
                break;
            case BUSINESS_TYPE_LOGIN_LOG:
                viewData.getCondition().put(SELECT_USER_ID,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D000));
                viewData.getCondition().put(SELECT_LOGIN_STATUS,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D002));
                break;
            case BUSINESS_TYPE_NOTICE:
                viewData.getCondition().put(SELECT_USER_ID,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D000));
                viewData.getCondition().put(SELECT_NOTICE_TYPE,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D008));
                viewData.getCondition().put(SELECT_NOTICE_STATUS,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D007));
                viewData.getCondition().put(SELECT_READ_STATUS,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D012));
                viewData.getCondition().put(SELECT_BUSINESS_TYPE,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D011));
                viewData.getCondition().put(SELECT_BUSINESS_SUB_TYPE,
                        RunDataConfig.DICTIONARY_CONDITION.get(new StringBuffer(userId).append(BLANK).toString()).get(DictionaryConst.D003));
                viewData.getCondition().get(SELECT_BUSINESS_SUB_TYPE).addAll(RunDataConfig.DICTIONARY_CONDITION.get(userId).get(DictionaryConst.D004));
                break;
            default:
                break;
        }
    ...
    public void initSystem() {
        ...
        RunDataConfig.SYSTEM_USED_STATUS = SYSTEM_STATUS_INIT;
        ...
        RunDataConfig.SYSTEM_USED_STATUS = null;
    }
    ...
    public void updateSystem() {
        ...
        if (!SYSTEM_VERSION.equals(currentVersion)) {
            RunDataConfig.SYSTEM_USED_STATUS = SYSTEM_STATUS_UPDATE;
    ...
    public void getConfigSql() {
        RunDataConfig.CONFIG_SQL = SysCommonUtils.getConfigSql(SYSTEM_CONFIG_SQL);
    }
    ...
    private void backup(String backupType) {
        ...
        RunDataConfig.SYSTEM_USED_STATUS = SYSTEM_STATUS_BACKUP;
        try {
    ...
    private void setDictionaryItem(SysUserModel sysUserModel,
        ...
        RunDataConfig.DICTIONARY_CONDITION.put(sysUserModel.getUserId(), codeMap);
        RunDataConfig.DICTIONARY_CONDITION.put(new StringBuffer(sysUserModel.getUserId()).append(BLANK).toString(), codeMapBlank);
    }
    ...
    private Boolean isUserDictionary(SysUserModel sysUserModel,
                                     ...
                                     Map<String, Boolean> userDataAuthority) {
        String dictionaryCode = sysDictionaryModel.getDictionaryCode();
        boolean flag = DictionaryConst.D000.equals(dictionaryCode) || DictionaryConst.D005.equals(dictionaryCode) || DictionaryConst.D009.equals(dictionaryCode);
