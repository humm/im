...
<script>
    layui.config({
    ...
    }).use(['index', 'admin', 'im', 'set'], function () {
        var $ = layui.$,
            form = layui.form,
            admin = layui.admin,
            im = layui.im;
        ...
        $(document).on('click', '.layui-btn', function () {
            ...
            if(im.isBlank(password) || password.length < 6 || password.length > 16 || password != repassword){
                return;
            }
            if(oldPassword != userPassword){
                im.msg(im.tips.msg.historyPasswordError);
                return;
            }
            if(password == userPassword){
                im.msg(im.tips.msg.historyPasswordEqualPassword);
                return;
            }
            var request = {
