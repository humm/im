...
<script>
    layui.config({
                    ...
                    if (response.bizResult) {
                        ...
                        if (!$.isEmptyObject(parameterList)) {
                            for (var i = 0; i < parameterList.length; i++) {
                                ...
                                item += '   isEdit="' + parameterList[i].isEdit + '" parameterOldValue="' + im.value(parameterList[i].parameterOldValue) + '">';
                                ...
                                var parameterValue = im.value(parameterList[i].parameterValue);
                                if ('' == parameterValue) {
                                    parameterValue = '&nbsp;';
                                }
                                item += '           <p class="layadmin-textimg im-padding-parameter">' + parameterValue + '</p>';
                                item += '       </div>';
                                item += '    </div>';
                                item += '</div>';
        ...
        $(document).on('dblclick', 'div.layui-col-md4', function () {
            ...
            if(isEdit != '1'){
                im.msg(im.tips.msg.parameterNotUpdate);
                return;
            }
            var request = {
            ...
            im.open({
                ...
                area:  [im.size.ten, im.size.eleven],
                btn: [im.tips.btn.save, im.tips.btn.cancel],
                resize: im.set.resize,
                yes: function (e, t) {
                    save(e, t, im.operate.update, request);
                }
            });
        });
        ...
        var save = function (e, t, type, request) {
            var iframe = window["layui-layer-iframe" + e],
                ...
                if(request.parameterType == 'date' && request.parameterExt != data.field[request.parameterCode].length){
                    im.msg(im.tips.msg.onlyLength.replace("S", request.parameterExt));
                    return;
                }
                // 复选框取值
                var checkValue = [];
