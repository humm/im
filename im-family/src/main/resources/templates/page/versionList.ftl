...
<script>
    layui.config({
    ...
    }).use(['index', 'admin', 'family'], function () {
        var $ = layui.$,
            admin = layui.admin,
            family = layui.family;
        ...
        var initVersion = function (data) {
            if(!$.isEmptyObject(data)){
                ...
                for (var i=0; i<data.length; i++) {
                    ...
                    if (family.isBlank($("." + versionDate).html())) {
                        var item  = '<li class="layui-timeline-item-family">';
        ...
        var addItem = function (type, content) {
            var item  = '<li>';
                if (type == '1') {
                    item += '<span class="layui-btn-family layui-btn-xs-family layui-bg-orange">&nbsp;功能&nbsp;</span>';
                } else if (type == '2') {
                    item += '<span class="layui-btn-family layui-btn-xs-family layui-bg-blue">&nbsp;优化&nbsp;</span>';
                } else if (type == '3') {
                    item += '<span class="layui-btn-family layui-btn-xs-family layui-bg-gray">&nbsp;修复&nbsp;</span>';
                }else if (type == '4') {
                    item += '<span class="layui-btn-family layui-btn-xs-family layui-bg-red">&nbsp;发版&nbsp;</span>';
                }
                item += '&nbsp;&nbsp;&nbsp;';
                item += content;
                item += '</li>';
