...
<script>
    layui.config({
    ...
    }).use(['index', 'admin', 'element', 'family'], function () {
        var $ = layui.$,
            admin = layui.admin,
        ...
        function initMenu(data) {
            ...
            if (!$.isEmptyObject(data)) {
                data.forEach(function (item) {
                    if (!family.isBlank(item.menuUrl)) {
                        if (item.menuUrl != '#') {
                            // 单页面菜单
                            ...
                            menu += '       <cite>' + item.menuTitle + '</cite>';
                            menu += '    </a>';
                            if(!family.isBlank(item.children)){
        ...
        function initChildrenMenu(data) {
            data.forEach(function (subItem) {
                if(!family.isBlank(subItem.children)){
        ...
        var save = function (e, t, type) {
            var iframe = window["layui-layer-iframe" + e],
                ...
                var request = {
                    user: family.isBlank(data.field["user"]) ? 'off' : data.field["user"],
                    tips: family.isBlank(data.field["tips"]) ? 'off' : data.field["tips"],
                    login: family.isBlank(data.field["login"]) ? 'off' : data.field["login"],
                    version: family.isBlank(data.field["version"]) ? 'off' : data.field["version"],
                    register: family.isBlank(data.field["register"]) ? 'off' : data.field["register"]
                };
                admin.req({
                    url: url.save,
                    type: "post",
