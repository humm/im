...
public class HepTodoController extends BaseController implements Initializable {
    ...
    private Map<String, String[]> color = new LinkedHashMap<String, String[]>(){{
        put("完成日期超期", new String[] {TIME_OVER_COLOR});
    ...
    Map<String, Integer> taskDescSort = new HashMap(){{
        put(NAME_BUTTON_COMMIT, 10);
        put(NAME_BUTTON_WAIT_MERGE, 20);
        put(NAME_BUTTON_TODAY, 30);
        put(NAME_BUTTON_WEEK, 40);
        put(NAME_BUTTON_FOCUS, 50);
        put(NAME_BUTTON_ADD_TOADY, 60);
        put(NAME_BUTTON_DEFECT, 70);
        put(NAME_BUTTON_HAS_OVERDUE, 80);
        put(NAME_BUTTON_OVERDUE, 90);
        put(NAME_BUTTON_SOME_ONE, 100);
        put(NAME_BUTTON_ONLY_ONE, 110);
        put(NAME_BUTTON_ERROR_VERSION, 120);
    }};
    ...
    public void doScriptCheck() {
        ...
        try {
            ...
            OutputUtils.info(notice, TaCommonUtils.getMsgContainTimeContainBeforeBr("文件检查完成，请查看检查结果"));
        } catch (Exception e) {
            String msg = e.getMessage();
            OutputUtils.info(notice, TaCommonUtils.getMsgContainTimeContainBeforeBr(msg));
        } finally {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                if (dayCompleteTipsInfo.containsKey(minCompleteByMarkYmd)) {
                    String deadLine = dayCompleteTipsInfo.get(minCompleteByMarkYmd);
                    item.setDeadLine(deadLine);
                    if (StringUtils.equals(NAME_BUTTON_TODAY, deadLine)) {
                        setTaskDescTotal(NAME_BUTTON_TODAY);
                    }
                }

                if (commitTag) {
                    setTaskDesc(item, NAME_BUTTON_COMMIT);
                } else if (completeTag) {
                    setTaskDesc(item, NAME_BUTTON_WAIT_MERGE);
                }
                if (finishDateError.contains(taskNumberIn)) {
                    setTaskDesc(item, NAME_BUTTON_OVERDUE);
                } else if (finishDateOver.contains(taskNumberIn)) {
                    setTaskDesc(item, NAME_BUTTON_HAS_OVERDUE);
                }
                if (taskName.contains(DEFECT_TAG)) {
                    setTaskDesc(item, NAME_BUTTON_DEFECT);
                }

                if (StringUtils.equals(appConfigDto.getHepTaskSameOne(), STR_TRUE) && StringUtils.equals(item.getAssigneeId(), item.getReviewerId())) {
                    sameAssigneeIdReviewerId.add(taskNumberIn);
                    setTaskDesc(item, NAME_BUTTON_SOME_ONE);
                }
                ...
                if (StringUtils.equals(queryType, only.getId())) {
                    ...
                    } else if (StringUtils.equals(queryType, NAME_BUTTON_WEEK)) {
                        if (!weekTodoTask.contains(taskNumberIn)) {
                            iterator.remove();
                            continue;
                        }
                    } else {
                        String level = item.getTaskDesc() == null ? STR_BLANK : item.getTaskDesc();
                        if (!level.contains(queryType)) {
                            iterator.remove();
                            continue;
                        }
                    }
    ...
    private void controlHepTaskOnlySelfTips (AppConfigDto appConfigDto, HepTaskDto hepTaskDto, List<String> cancelOnlySelf) {
        ...
        if (hepTaskOnlySelfMap.containsKey(customerFull)) {
            if (!hepTaskDto.getSprintVersionFull().startsWith(hepTaskOnlySelfMap.get(customerFull))) {
                setTaskDesc(hepTaskDto, NAME_BUTTON_ONLY_ONE);
            }
        }
    }
        ...
        if (frontPage()) {
            ...
            for (Map.Entry<String, Map<String, Integer>> entry : focusVersion.entrySet()) {
                ...
                for (int i=0; i<user.size(); i++) {
                    ...
                    if (StringUtils.equals(extendUserInfoNameToCode.get(item), EXTEND_USER_FRONT_CODE)) {
                        continue;
                    }
    ...
    private void controlHepTaskAppointVersionTips (AppConfigDto appConfigDto, HepTaskDto hepTaskDto, List<String> cancelErrorVersion) {
        ...
        if (hepTaskAppointVersionMap.containsKey(customerFull)) {
            ...
            if (taskVersion.startsWith(ver) && !StringUtils.equals(version, taskVersion)) {
                setTaskDesc(hepTaskDto, NAME_BUTTON_ERROR_VERSION);
            }
        }
    }
    ...
    private void setTaskDescTotal(String taskDesc) {
        if (taskDescTotal.containsKey(taskDesc)) {
            taskDescTotal.put(taskDesc, taskDescTotal.get(taskDesc) + 1);
        } else {
            taskDescTotal.put(taskDesc, 1);
    ...
    private void infoTaskList(TableView taskListIn, List<HepTaskDto> res, Set<String> dayTodoTask, Set<String> weekTodoTask, Set<String> finishDateError,
                              Set<String> focusVersionTask, Set<String> todayAddTask, List<String> focusDemand) {
        if (taskListIn == null) {
            return;
        }
        ...
        Platform.runLater(() -> {
            for (HepTaskDto hepTaskDto : res) {
                ...
                // 设置行
                initRowStyle(taskListIn, dayTodoTask,  weekTodoTask, finishDateError, focusVersionTask, todayAddTask, focusDemand);
            }
                    ...
                    protected void updateItem(HepTaskDto item, boolean empty) {
                        try {
                            ...
                            if (item != null && getIndex() > -1) {
                                ...
                                taskColor = initTestDataColor(taskColor);
...
;        }
    ...
    private void controlFocusVersionTips(boolean visible) {
        ...
        OutputUtils.info(focusVersionTipsLabel, "重点关注版本：");
        ...
        taskList.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue != null) {
                if (newValue instanceof HepTaskDto) {
                    ...
                    controlTooltip(appConfigDto, show, msg, 1050, 205);
                }
            }
        });
    ...
    private static int currentDataNum = 0;

    private String[] initTestDataColor(String[] item) {
        String[] taskColor = item;
        if (!CommonUtils.proScene()) {
    ...
    private void initTestData() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            item.put("create_time", "2024-09-02 22:59:59");
