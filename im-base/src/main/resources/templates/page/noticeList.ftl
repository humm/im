...
<script>
    layui.config({
    ...
    }).use(['index', 'admin', 'table', 'util', 'im'], function () {
        var $ = layui.$,
            im = layui.im,
            admin = layui.admin,
        ...
        // 阅读状态
        var readStatus = im.getUrlParameter("readStatus");
        ...
        // websocket订阅消息通知主题
        im.webSocket(url.websocketUrl, "notice", function (data) {
            if(!im.isBlank(data)){
