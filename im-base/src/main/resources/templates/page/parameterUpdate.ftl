...
<script>
    ...
    }).use(['index', 'form', 'im'], function () {
        var $ = layui.$,
            form = layui.form,
            im = layui.im;

        var parameterCode = im.getUrlParameter("parameterCode");
        var parameterValue = decodeURI(im.getUrlParameter("parameterValue"));
        var parameterOldValue = decodeURI(im.getUrlParameter("parameterOldValue"));
        var parameterType = im.getUrlParameter("parameterType");
        var parameterExt = im.getUrlParameter("parameterExt");
        var maxlength = im.isBlank(parameterExt) ? 150 : parameterExt;
        var item = "";
        if (parameterType == 'text' || im.isBlank(parameterType)) {
